...
-- 0x30 : reserved
-- 0x34 : Data signal of n_elements
...
architecture behave of krnl_vadd_control_s_axi is
    ...
    constant ADDR_N_ELEMENTS_DATA_0 : INTEGER := 16#34#;
    constant ADDR_N_ELEMENTS_CTRL   : INTEGER := 16#38#;
    ...
    signal int_n_elements      : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_N_ELEMENTS_DATA_0 =>
                        rdata_data <= RESIZE(int_n_elements(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    n_elements           <= STD_LOGIC_VECTOR(int_n_elements);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_N_ELEMENTS_DATA_0) then
                    int_n_elements(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_n_elements(31 downto 0));
                end if;
            end if;
        end if;
    end process;
